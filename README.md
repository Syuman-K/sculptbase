...
### 統合が失敗しないための仕組み(v0.3.0〜)

転写後のベース表面と `_joint` のスカートはほぼ同一面で重なるため、素直にブーリアンすると
交差が退化して穴だらけの結果になります（v0.2.0 の実データで発生）。v0.3.0 では:

- **統合オフセット** — `_joint` に覆われた本体頂点だけを法線の内側へ僅かに沈め、交差を横断的にする。沈んだ領域は `_joint` が覆うので出力表面には現れない
- **ソルバーの自動選択** — Manifold（高速・堅牢）を試し、境界エッジが残ったら Exact で再試行
- **穴埋めの強化** — `holes_fill` が扱えないねじれループは中心頂点を立てた扇形で確実に塞ぐ（リメッシュ後・統合前の両方に適用）

実データ検証: 従来は出力に境界エッジが 5,565〜8,855 本残りダボが消失していたところ、
v0.3.0 では**境界エッジ 0（完全水密）**、寸法もソースと一致（ダボが正しく出力される）。
...
- **統合オフセット** — 統合時に `_joint` の下の本体表面を沈める量（既定 0.5mm 相当）。統合に失敗する場合はここと分離マージンを広げる。
